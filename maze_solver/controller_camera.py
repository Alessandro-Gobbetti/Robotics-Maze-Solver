...
from .enums import WallState, FloodFillState, CallbackUsage, RotationVal, Position
...
class ControllerNode(Node):
    def __init__(self):
        super().__init__('controller_camera')
    ...
    def move(self):
        if self.current_pose is not None and self.READY:
                    ...
                    match(self.cur_rot):
                        case RotationVal.NINETY_LEFT:
                            if self.goal_theta is None:
                                theta = self.current_pose[2] + np.pi/2
                                if theta > np.pi:
                                    theta -= 2*np.pi
                                self.goal_theta = theta
                            self.rotate_in_place(self.goal_theta)
                        ...
                        case RotationVal.BACK:
                            if self.goal_theta is None:
                                theta = self.current_pose[2] + np.pi
                                if theta > np.pi:
                                    theta -= 2*np.pi
                                self.goal_theta = theta
                            self.rotate_in_place(self.goal_theta)
            ...
            self.get_logger().info("Waiting for odometry and proximity sensors to setup")


    def rotate_in_place(self, goal_theta):
        """
        ...
        if abs(self.angular_difference(goal_theta, self.current_pose[2])) < self.angular_threshold:
            ...
            cmd_vel = Twist()
            cmd_vel.angular.z = 6*(self.angular_difference(goal_theta, self.current_pose[2]))
